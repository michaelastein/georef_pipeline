...
def read_csv(csv_path, image_data_list):
    ...
    for _, row in df.iterrows():
        ...
        if csv_name in image_names_set:
            row_dict = {}
            if "center_x" in row:
                row_dict["center_x"] = safe_float(row["center_x"])
            if "center_y" in row:
                row_dict["center_y"] = safe_float(row["center_y"])
...
def add_anomaly_to_csv(output_array, anomaly_type, latitude, longitude,
    ...
    """
    Add an anomaly dict to output_array. No file_name is stored anymore.
    """
    entry = {
...
def process_next_anomaly(row, image_data_list, build_corr_func, csv_path, dem_path = None, lidar_path = None):
    ...
    try:
        ...
        idx = 0  # always use the first image since we removed file_name
        ...
        # Add anomaly including example image info
        add_anomaly_to_csv(
            output_array,
            anomaly_type,
            avg_lat,
            avg_lon,
            example_image=best_item.get("example_image"),
            example_pixel_x=best_item.get("example_pixel_x"),
            example_pixel_y=best_item.get("example_pixel_y")
        )
